...
function log_back_acc(ctx: Context,config: Config): any {
    if(config.log_msg == true){
        ...
        if(config.log_ct == true){
            ctx.on('message',(session: Session) =>{
                ctx.logger.info('\x1b[33msend message')
            })
        }
    }
}

function log_back_ruf(ctx: Context,config: Config): any {
    if(config.log_msg == true){
        if(config.log_all == true){
            ctx.on('message',(session: Session) =>{
        ...
        if(config.log_ct == true){
            ctx.on('message',(session: Session) =>{
                ctx.logger.info('\x1b[33mnot send message')
            })
        }
    }
}